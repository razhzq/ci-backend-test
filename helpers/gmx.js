...
const gmxPosabiPath = path.resolve(__dirname, "../contractABI/GMXPositionRouter.json");  
...
module.exports.getPairPriceGMX = async (asset) => {
    ...
    let price;
    try {
        await axios.get('https://api.gmx.io/prices').then((p) => price = p.data[pairContract])
        return price;
    } catch (error) {
        console.log(error)
...
module.exports.createPositionGMX = async (privateKey, indexToken, collateralAmount, isLong, price, leverage) => {
    const account = web3.eth.accounts.privateKeyToAccount(privateKey);

    const routerContract = new web3.eth.Contract(gmxRouterAbi, gmxRouterAddress);
    const positionRouterContract = new web3.eth.Contract(gmxPosRouterAbi, gmxPosRouterAddress);
    const daiContract = new web3.eth.Contract(daiAbi, daiAddress);

    const collateralConv = await web3.utils.toWei(collateralAmount.toString(), 'ether');
    const sizeDelta = collateralAmount * leverage * (10 ** 30);

    try {
        await daiContract.methods.approve(gmxPosRouterAddress, collateralConv).send({from: account, gasLimit: '5000000', transactionBlockTimeout: 200 })
           .on('transactionHash', (hash) => {
                 routerContract.methods.approvePlugin(gmxPosRouterAddress).send({from: account, gasLimit: '5000000', transactionBlockTimeout: 200 })
                 .on('transactionHash', (hash) => {
                 ...
                 }).send({from: account, gasLimit: '5000000', transactionBlockTimeout: 200 })
                 .on('receipt', receipt => {
...
module.exports.closePositionGMX = async (privateKey, asset, collateral, sizeDelta, isLong, receiver, price) => {
    const account = web3.eth.accounts.privateKeyToAccount(privateKey);
    const positionRouterContract = new web3.eth.Contract(gmxPosRouterAbi, gmxPosRouterAddress);
    ...
    try {
        await positionRouterContract.methods.createDecreasePosition(
            daiAddress,
            indexToken,
            collateral,
            sizeDelta,
