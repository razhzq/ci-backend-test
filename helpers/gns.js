...
const gnsTradingabiPath = path.resolve(__dirname, "../contractABI/GNSTradingContract.json");  
...
module.exports.getGnsPairPrice = async (asset) => {
    ...
    if(pair?.network == 'arbitrum') {
        ...
        contract = new web3Polygon.eth.Contract(pricecontractAbi, pair?.contractAddress);
     }
...
module.exports.openTradeGNS = async (privateKey, network, pairIndex, positionSizeDai, openPrice, isLong, leverage, tp, sl) => {
    ...
    await daiContract.methods.approve('', 'amount').send({from: account, gasLimit: '5000000', transactionBlockTimeout: 200}).then((transactionHash) => {
        tradingContract.methods.openTrade(tradeTuple, 0, 0, '30000000000', '0x0000000000000000000000000000000000000000' ).send({from: account, gasLimit: '5000000', transactionBlockTimeout: 200})
    } )
